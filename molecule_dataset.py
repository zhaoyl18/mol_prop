...
ZINC250K_PATH = "/data/yulai/projects/molecule-fine-tuning/data/250k_rndm_zinc_drugs_clean_3.csv"
...
class ZINCDataset(torch.utils.data.Dataset):
	def __init__(self, label_method=None, **label_kwargs):
		...
		self.all_qed =  zinc_table["qed"] 
		self.all_SAS = zinc_table["SAS"]
		self.all_logp = zinc_table["logP"]
	...
	def __getitem__(self, index):
		...
		return torch.cat([adj, x], dim=1), self.target[index], self.all_qed[index], self.all_logp[index], self.all_SAS[index]
